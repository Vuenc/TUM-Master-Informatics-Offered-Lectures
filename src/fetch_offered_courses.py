...
if __name__ == "__main__":
    ...
    available_courses_dtos = sum(
        tqdm.contrib.concurrent.thread_map(
            fetch_courses_for_term, reversed(terms), max_workers=len(terms)),
        []
    )

    with open("../data/all_offered_courses.json", "w") as f:
        json.dump(available_courses_dtos, f)

    print("Results written to JSON file ../data/all_offered_courses.json")
